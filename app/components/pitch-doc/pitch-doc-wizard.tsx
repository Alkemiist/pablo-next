...
interface PitchDocWizardProps {
  onComplete: (data: PitchDocIntake) => void;
  onClose: () => void;
  initialData?: PitchDocIntake;
}

export default function PitchDocWizard({ onComplete, onClose, initialData }: PitchDocWizardProps) {
  const [currentStep, setCurrentStep] = useState(0);
  const [formData, setFormData] = useState<PitchDocIntake>({
